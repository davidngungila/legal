...
@section('title', 'HR Competency Interview Assessment - Orvion HRIS')

@section('content')
<div class="p-6 max-w-6xl mx-auto">
    <!-- Header -->
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900 font-manrope">HR Competency Interview Assessment</h1>
        <p class="text-gray-600 mt-2">Conduct comprehensive competency interview assessment for candidate evaluation</p>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200">
        <form id="hrInterviewForm" class="p-6 space-y-8">
            ...
            <div class="border-b border-gray-200 pb-8">
                ...
                <div class="mb-8">
                    <h3 class="text-lg font-medium text-gray-800 mb-4">Functional Competencies</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        @foreach([
                            ...
                            'teamwork' => 'Teamwork',
                            'adaptability_flexibility' => 'Adaptability/Flexibility'
                        ] as $field => $label)
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">
                                ...
                                <span class="text-red-500 text-sm hidden" id="{{ $field }}_error"></span>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">
                                    Comment
                                </label>
                                <textarea name="{{ $field }}_comment" rows="2"
                                          class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"></textarea>
                            </div>
                        @endforeach
                    </div>
                ...
                <div class="mb-8">
                    <h3 class="text-lg font-medium text-gray-800 mb-4">Managerial Competencies</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        @foreach([
                            'leadership' => 'Leadership',
                            'managing_developing_people' => 'Managing & Developing People',
            ...
            <div class="border-b border-gray-200 pb-8">
                <h2 class="text-xl font-semibold text-gray-900 mb-6">Overall Assessment</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">
                        ...
                        <span class="text-red-500 text-sm hidden" id="overall_rating_error"></span>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">
                    ...
                    <div>
                        ...
                        <textarea name="main_strength" rows="2"
                                  class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"></textarea>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">
                        ...
                        <textarea name="main_weakness" rows="2"
                                  class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"></textarea>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">
...
@push('scripts')
<script>
// HR Competency Interview Management System
class HrInterviewManager {
    ...
    setupEventListeners() {
        ...
        // Rating validation
        const ratingSelects = this.form.querySelectorAll('select[name$="_rating"], select[name^="education_job_knowledge"], select[name^="relevant_job_experience"], select[name^="major_previous_achievement"], select[name^="language_fluency"], select[name^="interactive_communication"], select[name^="accountability"], select[name^="work_excellence"], select[name^="functional_competencies"], select[name^="planning_organizing"], select[name^="problem_solving"], select[name^="attention_to_details"], select[name^="multitasking"], select[name^="continuous_improvement"], select[name^="compliance"], select[name^="creative_innovation"], select[name^="negotiation"], select[name^="teamwork"], select[name^="adaptability_flexibility"], select[name^="leadership"], select[name^="managing_developing_people"], select[name^="managing_change"], select[name^="making_decisions"]');
    ...
    async submitForm() {
        ...
        try {
            ...
            const response = await fetch('/hr-interview', {
                method: 'POST',
                headers: {
            ...
            if (result.success) {
                this.showNotification('HR competency interview assessed successfully!', 'success');
                setTimeout(() => {
...
function saveAsDraft() {
    ...
    localStorage.setItem('hrInterviewDraft', JSON.stringify(data));
    
    window.hrInterviewManager.showNotification('Draft saved successfully', 'success');
}
