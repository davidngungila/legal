        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-semibold text-gray-900" id="avgSalary">-</p>
                </div>
            </div>
        </div>
    </div>
                    ...
                    @forelse($employees as $employee)
                        <tr class="hover:bg-gray-50 transition-colors employee-row" 
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <div class="text-xs text-gray-400">2024-01-15 to 2025-01-15</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">$5,500/month</div>
                                <div class="text-sm text-gray-500">Bank Transfer</div>
                                <div class="text-xs text-green-600">Total: $6,200</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">40 hrs/week</div>
                                <div class="text-sm text-gray-500">21 days leave</div>
                                <div class="text-xs text-blue-600">Full benefits</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">
                                    Active
                                </span>
                                <div class="text-xs text-gray-500 mt-1">11 months remaining</div>
                            </td>
    ...
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-lg bg-white">
        <div class="mt-3">
            ...
            <div class="space-y-4">
                ...
                <div class="flex justify-between items-center">
                    <span class="text-sm text-gray-600">Termination Rate:</span>
                    <span class="text-sm font-medium" id="modalTerminationRate">-</span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-sm text-gray-600">Average Salary:</span>
                    <span class="text-sm font-medium" id="modalAvgSalary">-</span>
                </div>
            </div>
...
<div id="terminateModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-lg bg-white">
        <div class="mt-3">
            <h3 class="text-lg font-medium text-gray-900 mb-4">Terminate Employment Contract</h3>
            <form id="terminateForm" class="space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Final Pay Date <span class="text-red-500">*</span></label>
                    <input type="date" name="final_pay_date" required
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                </div>
                <div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" name="reference_letter_provided" id="reference_letter_provided"
                               class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded">
                        <label for="reference_letter_provided" class="ml-2 block text-sm text-gray-900">
...
@push('scripts')
<script>
// Employment Contracts Management System
class EmploymentContractsManager {
    constructor() {
        this.init();
    }
    ...
    async loadStatistics() {
        try {
            const response = await fetch('/employment-contracts/statistics');
            ...
            if (result.success) {
                ...
                document.getElementById('avgSalary').textContent = '$' + stats.average_salary.toLocaleString();
                ...
                document.getElementById('modalTerminationRate').textContent = stats.termination_rate + '%';
                document.getElementById('modalAvgSalary').textContent = '$' + stats.average_salary.toLocaleString();
    ...
    async terminateContract() {
        ...
        data.reference_letter_provided = form.querySelector('#reference_letter_provided').checked;

        this.setTerminateLoadingState(true);

        try {
            const response = await fetch(`/employment-contracts/${data.employee_id}/terminate`, {
                method: 'POST',
                headers: {
...
async function showRequiringAttentionModal() {
    try {
        const response = await fetch('/employment-contracts/requiring-attention');
        const result = await response.json();
        ...
        if (result.success) {
            ...
            window.employmentContractsManager.showNotification('Failed to load requiring attention contracts', 'error');
        }
    } catch (error) {
        console.error('Failed to load requiring attention contracts:', error);
        window.employmentContractsManager.showNotification('An error occurred', 'error');
    }
}
...
async function showCalendarModal() {
    try {
        const response = await fetch('/employment-contracts/calendar');
        const result = await response.json();
        ...
        if (result.success) {
            ...
            window.employmentContractsManager.showNotification('Failed to load calendar events', 'error');
        }
    } catch (error) {
...
function showEmployeeContracts(employeeId) {
    window.location.href = `/employment-contracts/employee/${employeeId}`;
}

function renewContract(employeeId) {
    window.employmentContractsManager.showNotification('Contract renewal feature coming soon', 'info');
}
...
function generatePdf(employeeId) {
    window.employmentContractsManager.showNotification('Contract PDF generation feature coming soon', 'info');
}

// Initialize employment contracts manager
document.addEventListener('DOMContentLoaded', function() {
    window.employmentContractsManager = new EmploymentContractsManager();
});
</script>
@endpush
